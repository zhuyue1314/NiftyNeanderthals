...
# Usage: check-blacklist.py -f /data/145.0.2.0_24.json -b /data/sslblacklist.csv
...
def parse_input(INFILE, BLACKLIST):
    """Parsing input file"""
    TSUBNET = basename(INFILE)
    TEMP = TSUBNET.split('.')
    SUBNET = str(TEMP[0]) + "." + str(TEMP[1]) + "." + str(TEMP[2])
    ips = pull_ips(INFILE, SUBNET)
    bad_hashes = get_bad_cert_hashes(BLACKLIST)
    print "[INFO] Input file: " + INFILE
    print "[INFO] IP Range: " + SUBNET
